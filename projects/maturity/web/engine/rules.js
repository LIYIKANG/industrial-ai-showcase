 ...
 */
import { CARDS, COPY, CORE_QUESTIONS, DIMS, INDUSTRIES, INDUSTRY_NAME, PAINS, SIZES, } from './config.js';
export function classifyIndustry(freeText) {
...
function optScore(answers, questionId) {
    const question = PAINS.find((p) => p.followUp.id === questionId)?.followUp ??
...
const CARD_RULES = [
    {
        ...
        weight: (input) => {
            let w = 0;
            if (input.painIds.includes('p_data'))
                w += 2;
            if (input.quickWins.includes('qw_report'))
                w += 2;
            if (optScore(input.answers, 'M2') >= 0 && optScore(input.answers, 'M2') <= 1)
                w += 1;
            return w;
        },
    ...
    },
    {
        ...
        weight: (input) => {
            let w = 0;
            if (input.painIds.includes('p_knowledge'))
                w += 2;
            if (input.quickWins.includes('qw_know'))
                w += 2;
            if (input.quickWins.includes('qw_qa'))
                w += 2;
            return w;
        },
        prereqMet: (input) => {
    ...
    },
    {
        ...
        weight: (input) => {
            let w = 0;
            if (input.painIds.includes('p_quality'))
                w += 2;
            if (input.quickWins.includes('qw_trace'))
                w += 2;
            return w;
        },
        prereqMet: (input) => {
    ...
    },
    {
        ...
        weight: (input) => {
            let w = 0;
            if (input.painIds.includes('p_delivery'))
                w += 2;
            if (input.painIds.includes('p_inventory'))
                w += 2;
            return w;
        },
        prereqMet: (input) => optScore(input.answers, 'D2') >= 2 || optScore(input.answers, 'f_inventory') >= 2,
    },
    {
        id: 'card_doc',
        weight: (input) => {
            let w = 0;
            if (input.quickWins.includes('qw_doc'))
                w += 2;
            if (input.quickWins.includes('qw_qa'))
                w += 1;
            return w;
...
export function buildCards(input, result) {
...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
export function buildConfidenceFlags(input, result) {
    const flags = [];
    ...
    if (m2 === 3 && d2 <= 1 && d2 >= 0) {
        flags.push('您提到经营指标可实时看到，但同时表示数据账实经常不符——看板数据的可信度值得先复核。');
    }
    if (input.painIds.includes('p_data') && d1 === 3 && s2 >= 2) {
        flags.push('您选择了「数据分散」的痛点，但系统化与集成程度的作答较高，两者略有出入，建议现场访谈时再确认。');
    }
    if (o1 === 3 && (result.dims.D + result.dims.S) / 2 < 35) {
        flags.push('高层投入意愿很强，但数据与系统基础尚薄弱——建议先以小场景验证再放大投入，避免期望与地基错位。');
    }
    return flags;
}
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
export function buildExpertText(input, result, cards, flags) {
    ...
    const dimIds = Object.keys(result.dims);
    ...
    const parts = [];
    parts.push(`作为一家${sizeLabel ? `${sizeLabel}规模的` : ''}${industryName}企业，贵司整体 AI 就绪度处于「${result.tier.name}」（总分 ${result.totalScore}，${result.positionText}）。`);
    if (result.dims[strongest] !== result.dims[weakest]) {
        parts.push(`相对优势在${DIMS[strongest].name}（${result.dims[strongest]} 分），当前的主要短板是${DIMS[weakest].name}（${result.dims[weakest]} 分）——它很可能是后续 AI 场景能否跑通的约束项。`);
    }
    else {
        parts.push(`四个维度得分较为均衡，说明基础没有明显偏科，适合按价值优先级选择切入场景。`);
    }
    if (painLabels.length > 0) {
        parts.push(`您提到的「${painLabels.join('」「')}」，在同行业中相当典型；下方推荐的场景正是针对这些问题给出的切入路径。`);
    }
    if (cards.length > 0) {
        parts.push(`结合您希望优先见效的环节，建议从「${cards[0].title}」入手，三个月内先跑出一个可衡量的闭环，再逐步扩展。`);
    }
    if (flags.length > 0) {
