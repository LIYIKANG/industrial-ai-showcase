...
export type DimId = 'D' | 'S' | 'M' | 'O';

export const DIMS: Record<DimId, { name: string; short: string }> = {
...
/** 总分中各维度的权重（和为 1） */
export const DIM_WEIGHTS: Record<DimId, number> = {
  D: 0.3,
  S: 0.25,
  M: 0.25,
  O: 0.2,
...
export interface Question {
  ...
  /** 计入第 5 轴「AI 行动力」（仅组织维度的 AI 相关题） */
  ai?: boolean;
}

/* --------------------------------------------------------------------------
   行业与关键词归类（LLM 分类的本地降级方案）
   -------------------------------------------------------------------------- */
...
export const INDUSTRY_NAME: Record<IndustryId, string> = Object.fromEntries(
...
/* --------------------------------------------------------------------------
   痛点（多选，至多 3 项）与对应追问 —— 选 N 项，紧接着出 N 道追问
   -------------------------------------------------------------------------- */
...
/* --------------------------------------------------------------------------
   快赢环节识别（多选，位于评估题之后、报告之前；「说不上来」与其他互斥）
   -------------------------------------------------------------------------- */

export interface QuickWinOption {
  id: string;
  label: string;
  /** 与其他选项互斥 */
  exclusive?: boolean;
...
export const QUICK_WIN_QUESTION = {
  ...
  ] satisfies QuickWinOption[],
} as const;

/* --------------------------------------------------------------------------
   行业基准（经验估计值，用于相对位置的正态近似）
   -------------------------------------------------------------------------- */
...
export const BENCHMARKS: Record<IndustryId, Benchmark> = {
...
export const COPY = {
  ...
  disclaimer:
    '本报告由快评作答自动生成，行业基准为经验估计值，相对位置为近似测算，仅供决策参考；具体路径以现场诊断为准。',
