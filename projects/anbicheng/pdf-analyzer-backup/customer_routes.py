...
from core.customer_word_exporter import write_customer_result_word
...
def _write_job_export(
    *,
    ...
    write_customer_result_workbook(
        file_id=file_id,
...
async def download_customer_file(
    ...
    format: str = Query("excel", regex="^(excel|word)$"),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    ...
    payload = _parse_job_payload(job)
    config = load_customer_keyword_config(payload.get("config_id"))
    source_filename = payload.get("original_filename") or job.filename

    if format == "word":
        word_path = write_customer_result_word(
            file_id=file_id,
            fields=payload.get("fields", []),
            line_items=payload.get("line_items", []),
            business_result=payload.get("business_result", {}),
        ...
        word_filename = filename.rsplit(".", 1)[0] + ".docx" if "." in filename else filename + ".docx"
        return FileResponse(
            path=str(word_path),
