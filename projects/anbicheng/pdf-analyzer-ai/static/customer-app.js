(function () {
  ...
  function renderQueue(batch) {
      ...
      return (
        '<tr class="queue-row' + activeClass + '">' +
          ...
          '<td><div class="queue-actions">' +
            ...
            '<button class="text-button queue-download" type="button" data-id="' + escapeHtml(job.file_id) + '" data-filename="' + escapeHtml(job.output_filename || "customer_result.xlsx") + '"' + (canOpen ? "" : " disabled") + '>下载</button>' +
          '</div></td>' +
        '</tr>'
      );
    }).join("");
    ...
    body.querySelectorAll(".queue-download").forEach(function (button) {
      button.addEventListener("click", function () {
        downloadJob(button.dataset.id, button.dataset.filename || "customer_result.xlsx");
      });
    });
  }
  ...
  async function downloadFile() {
    ...
    return downloadJob(state.result.file_id || state.result.job_id, state.result.filename || "customer_result.xlsx");
  }

  async function downloadJob(fileId, filename) {
    if (!fileId) return;
    var url = "/api/customer/download/" + encodeURIComponent(fileId) + "?filename=" + encodeURIComponent(filename);
    try {
