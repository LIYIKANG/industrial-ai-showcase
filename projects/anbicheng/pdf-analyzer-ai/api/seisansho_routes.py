"""
api/seisansho_routes.py
========================
精算書（AD相殺書）処理用 API ルート。

ルート：
  POST /api/seisansho/process              → PDF/画像をアップロード、ジョブ登録（202）
  GET  /api/seisansho/jobs/{job_id}        → ジョブ状態ポーリング
  GET  /api/seisansho/download/{file_id}  → 生成済み Excel をダウンロード
  POST /api/seisansho/regenerate          → 手動修正後に Excel を再生成
"""
...
from core.config import SEISANSHO_CONFIG_PATH, load_config
...
seisansho_router = APIRouter()
logger = logging.getLogger(__name__)


async def _run_seisansho_job_bg(
    job_id: str,
    ...
    try:
        ...
        logger.info("[%s] 精算書 処理開始 files=%d user=%s", job_id, len(file_data), username)
        db.commit()
        ...
        logger.info("[%s] 精算書 完了 filled=%d/%d", job_id, filled_count, total_count)
...
@seisansho_router.post("/api/seisansho/process", status_code=202)
async def process_seisansho(
    background_tasks: BackgroundTasks,
    pdf_files: List[UploadFile] = File(...),
    _current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """精算書 PDF/画像を受け取り、ジョブ ID を即座に返す。AI 抽出・Excel 生成はバックグラウンドで実行。"""
    for f in pdf_files:
    ...
    try:
        current_config = load_config(SEISANSHO_CONFIG_PATH)
        ...
        out_filename = (
            f"seisansho_{Path(file_data[0][0]).stem}.xlsx"
            if len(file_data) == 1
            else f"seisansho_merged_{len(file_data)}files.xlsx"
        )
...
@seisansho_router.get("/api/seisansho/jobs/{job_id}")
async def get_seisansho_job_status(
    job_id: str,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """精算書ジョブ状態を返す。done の場合は抽出結果も含む。"""
    job = db.query(ProcessingJob).filter(ProcessingJob.file_id == job_id).first()
    if not job:
...
@seisansho_router.get("/api/seisansho/download/{file_id}")
async def download_seisansho(
    file_id: str,
    filename: str = "seisansho.xlsx",
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """生成済み精算書 Excel をダウンロード。ファイルが消えていれば DB から再生成。"""
...
@seisansho_router.post("/api/seisansho/regenerate")
async def regenerate_seisansho(
    payload: Dict[str, Any],
    _current_user: User = Depends(get_current_user),
):
    """手動修正後に精算書 Excel を再生成。"""
    try:
        current_config = load_config(SEISANSHO_CONFIG_PATH)
        ...
        filename = payload.get("filename", "seisansho_manual.xlsx")
